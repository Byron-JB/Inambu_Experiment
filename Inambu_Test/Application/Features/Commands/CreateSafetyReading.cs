...
{
    public record CreateSafetyReadingCommand(SafetyCaptureDTO SafetyCapture) : IRequest<CommandResult>;


    public class CreateSafetyReadingCommandHandler : IRequestHandler<CreateSafetyReadingCommand, CommandResult>
    {
        private readonly IMeasurement _measurement;
        private readonly IProductionLine _productionLine;

        public CreateSafetyReadingCommandHandler(IMeasurement measurement,IProductionLine productionLine)
        {
            _measurement = measurement;
            _productionLine = productionLine;
        }

        public async Task<CommandResult> Handle(CreateSafetyReadingCommand request, CancellationToken cancellationToken)
        {
